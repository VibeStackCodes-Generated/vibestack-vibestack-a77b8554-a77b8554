...
export function AssignmentList({ items, onEdit, onDelete }: AssignmentListProps) {
  if (items.length === 0) {
        ...
        <Card key={item.id}>
          <CardContent className="flex items-center justify-between p-4">
            <div className="flex-1">
              ...
              <p className="text-sm text-[hsl(var(--muted-foreground))]">{String(item["assignment_role"])}</p>
            </div>
