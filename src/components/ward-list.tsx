...
interface WardListProps {
  items: Ward[];
  onEdit?: (item: Ward) => void;
  onDelete?: (id: string) => void;
}

export function WardList({ items, onEdit, onDelete }: WardListProps) {
  if (items.length === 0) {
        ...
        <Card key={item.id}>
          <CardContent className="flex items-center justify-between p-4">
            <div className="flex-1">
              ...
              <p className="text-sm text-[hsl(var(--muted-foreground))]">{String(item["floor"])}</p>
            </div>
